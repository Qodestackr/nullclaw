...
`POST /media/transcribe` accepts JSON with `audio_base64`, `mime_type`, `source`, and optional `language`, then returns `{"text":"..."}` plus metadata. Configure STT under `tools.media.audio`; raise `gateway.max_body_size_bytes`, `gateway.request_timeout_secs`, and `gateway.webhook_rate_limit_per_minute` for live desktop audio chunking.
